...
from PyQt5.QtCore import Qt,QStandardPaths # 引入QStandardPaths
...
class showImage:  # 定义一个名为showImage的类
    ...
    def __init__(self):  # 类的初始化方法
        # 从文件中加载UI定义
        self.ui = uic.loadUi("resources/ui/main.ui")  # 加载名为main.ui的UI文件
    ...
    def openimage(self):  # 定义openimage方法
        ...
        desktop_path = QStandardPaths.standardLocations(QStandardPaths.DesktopLocation)[0] # 获取桌面路径
        file_path, _ = file_dialog.getOpenFileName(None, '选择图片', desktop_path, 'Images (*.png *.xpm *.jpg *.bmp)')  # 获取用户选择的图片文件路径
...
if __name__ == '__main__':
    ...
    app.setWindowIcon(QIcon('resources/images/2.bmp'))  # 设置窗口图标
